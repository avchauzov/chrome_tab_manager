...
export const HOSTNAME_GROUP_COLOR = 'blue';
...
async function createHostnameGroup(windowId, tabIds, hostname) {
  ...
  if (groupId !== null) {
    await safeTabGroupsUpdate(groupId, { title: hostname, color: HOSTNAME_GROUP_COLOR });
  }
  return groupId;
}
...
async function addTabsToHostnameGroup(groupId, hostnameTabs) {
  ...
  await safeTabsGroup({ groupId, tabIds: toAdd });
  return toAdd.length;
}
...
async function groupHostnameTabs(windowId, hostname, hostnameTabs) {
  ...
  if (remaining.length > 0) {
    await safeTabsGroup({ groupId: createdGroupId, tabIds: remaining });
  }
  await addActionLog('group', `created group: ${hostname}, ${hostnameTabs.length} tabs`);
  return 1;
}
