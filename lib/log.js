...
export async function setLastStatus(summary) {
  await safeStorageLocalSet({ lastActionSummary: summary });
}
...
export function formatSummary(summary) {
  ...
  if (summary.stale > 0) parts.push(`moved ${summary.stale} tab${summary.stale === 1 ? '' : 's'} to Stale`);
